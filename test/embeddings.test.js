...
test("a persistent 429 is NOT split into one request per item", async () => {
  ...
  assert.equal(calls.length, 2, "the attempt plus what the patience budget affords, no fan-out");
});
...
test("a rate limit is given time, not a handful of attempts", async () => {
  ...
  assert.equal(calls.length, 5, "four refusals then the answer, on the time budget");
});

test("the patience budget is finite, so a permanently throttled key still reports", async () => {
  const calls = stubProvider({ fails: 99, retryAfter: 0 });
  await assert.rejects(
    () => embedTexts(["a"], { ...cfg, rateLimitWaitMs: 50, rateLimitBudgetMs: 120 }),
    /429/,
  );
...
test("one 429 paces every later request, instead of bursting into the next window", async () => {
  ...
  assert.equal(calls.length, 4, "one refusal, its retry, then the two remaining batches");
  assert.equal(rateLimitState().pacingMs, 120, "the learned gap survives the batch");
  const gap = calls[3].at - calls[2].at;
  assert.ok(gap >= 110, `expected the learned pacing between requests, saw ${gap}ms`);
});
...
test("a 429 is survived with hedging ON, which is the shipped default", async () => {
  ...
  assert.equal(vecs.length, 1);
  assert.equal(calls.length, 2, "the refusal and its retry, with no hedge fired");
});
...
test("an item longer than the character cap is sent alone rather than dropped", () => {
  const plan = planBatches(["x".repeat(500), "y"], { batchSize: 16, maxCharsPerRequest: 100 });
